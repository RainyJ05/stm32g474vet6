...
// SPI接口发送多个字节, pBuffer是发送数据缓存区指针，byteCount是发送数据字节数，byteCount最大256
HAL_StatusTypeDef SPI_TransmitBytes_DMA(uint8_t *pBuffer, uint16_t byteCount)
{
...
// SPI接口接收多个字节， pBuffer是接收数据缓存区指针，byteCount是需要接收数据的字节数
HAL_StatusTypeDef SPI_ReceiveBytes_DMA(uint8_t *pBuffer, uint16_t byteCount)
{
...
// 从任何地址开始读取指定长度的数据
// globalAddr：开始读取的地址(24bit)， pBuffer：数据存储区指针，byteCount:要读取的字节数
void Flash_ReadBytes_DMA(uint32_t globalAddr, uint8_t *pBuffer, uint16_t byteCount)
{
    uint8_t byte2, byte3, byte4;
    Flash_SpliteAddr(globalAddr, &byte2, &byte3, &byte4); // 24位地址分解为3个字节
    ...
    SPI_TransmitOneByte(byte3);
    SPI_TransmitOneByte(byte4);
    SPI_ReceiveBytes_DMA(pBuffer, byteCount); // 接收byteCount个字节数据
}
...
// pBuffer是要写入数据缓冲区指针，byteCount是需要写入的数据字节数
// 写入的Page必须是前面已经擦除过的，如果写入地址超出了page的边界，就从Page的开头重新写
void Flash_WriteInPage_DMA(uint32_t globalAddr, uint8_t *pBuffer,
                           uint16_t byteCount)
{
    uint8_t byte2, byte3, byte4;
    Flash_SpliteAddr(globalAddr, &byte2, &byte3, &byte4); // 24位地址分解为3个字节
    ...
    SPI_TransmitOneByte(byte3);
    SPI_TransmitOneByte(byte4);
    SPI_TransmitBytes_DMA(pBuffer, byteCount); // 发送byteCount个字节的数据
    //	for(uint16_t i=0; i<byteCount; i++)
    //	{
    //		byte2=pBuffer[i];
    //		SPI_WriteOneByte(byte2);	//要写入的数据
